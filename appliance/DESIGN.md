...
- `CONFIG_PERF_EVENTS=y` (115 KB) -- kept because removing it makes `perf_event_open()` return `ENOSYS`, which fails `perf record`/`top`/`stat` and every sampling profiler outright, including async-profiler's perf-backed modes. This matters on macOS specifically: there the appliance *is* the kernel every container shares, and the root `DESIGN.md` commits to `perf` working inside instances as a stated difference from Docker.
  ...
  One caveat, and a bug rather than a design choice: the live `kernel.perf_event_paranoid` is `2`, the kernel default. The root `DESIGN.md` claims containers relax it to `1`, but nothing in the tree sets it (and it is not namespaced, so Incus cannot set it per instance). The practical effect is that userspace-only profiling works while kernel-inclusive profiling is refused with `EACCES`. Worth fixing separately -- it narrows what `PERF_EVENTS` delivers today, but does not change the case for keeping it.

Together these two cost about 389 KB of the *download* (the raw `.data` figures compress well), which is the number to weigh -- not the ~1 MB they occupy in the uncompressed image.
