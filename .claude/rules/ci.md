...
**vfkit cannot run on GitHub-hosted macOS runners, and this is structural.** Every hosted macOS runner, the paid `-large`/`-xlarge` ones included, is itself a macOS VM, and Virtualization.framework offers nested virtualization only to *Linux* guests. A probe on `macos-14`, `macos-15`, `macos-26` and `macos-15-intel` failed with `VZErrorDomain` "Virtualization is not available on this hardware" (Intel: "failed to start") before the guest ran any code, and third-party per-minute macOS runners are VMs too (#774). Testing vfkit needs a **bare-metal** Mac, as a self-hosted runner that gates releases (tags / `workflow_dispatch` behind a protected environment, never PRs). Until one exists, the manual pre-release run of `appliance/test-boot.sh` and `appliance/test-with-isx.sh` on a Mac is the gate. #791 is the Mac verification plan and the brief for that runner.

**What the vsock-bridged boot covers.** The topology is `Unix socket → [tunnel] → AF_VSOCK → in-guest socat forwarder → incusd`, and only `[tunnel]` is vfkit, so `integration-tests` reproduces everything either side of it with `socat UNIX-LISTEN:…,fork,backlog=128 VSOCK-CONNECT:3:<port>` on the paths isx looks for (`vm.incus.sock`, `vm.agent.sock`). `appliance/test-tunnel.sh` (shared with `test-boot.sh`'s vfkit path) then checks: the Incus API through the forwarder; every `isx-agent` verb and its argument validation; that 16 and 32 concurrent connections are all accepted and every forwarder child exits afterwards (the baseline `isx doctor`'s leak count assumes); that the `socat -T 180` backstop reaps a silent connection no earlier than the floor and the API recovers; and no-reboot `forwarder-restart`. The native `isx` then connects through the same socket: the runner has no native Incus, so `IncusApi.tryConnect` falls through to the vsock candidate exactly as on macOS. It does **not** prove vfkit itself: host socat propagates close faithfully, which is the property vfkit lacks. The job runs in parallel with `isx-integration-tests-native`, which is longer, so the ~3 minutes of backstop wait adds no wall-clock to the workflow.
...
Note that `isx init` is not run against the QEMU appliance VM: on Linux it provisions a *native* Incus, and the appliance has its own provisioning. The vsock-bridged boot in `integration-tests` exercises the client path to the appliance instead (`isx` over `vm.incus.sock`), which needs no `init`; `appliance/test-with-isx.sh` stays macOS-only because it has isx *boot* the VM, and isx's QEMU backend deliberately wires no vsock (Linux users run native Incus). The appliance also provisions Incus with its own shell script and never
