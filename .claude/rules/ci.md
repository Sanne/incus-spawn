...
The practical consequence: vfkit itself, the in-VM `socat` forwarder and `isx-agent` -- the guest half of the path macOS users actually run -- have **no automated coverage**, and aarch64 boots are covered only by `appliance/test-boot.sh`, which is developer-run. A green CI is not evidence that an appliance change works on macOS. The *host* half is covered: the Java compensations for vfkit's behaviour (exec completion via `/wait`, the adaptive drain, per-fd keepalive pings, the watchdogs, stale-connection recycling, the fast `tryConnect` probe) run in `unit-tests` against `LossyIncusServer`, a fake daemon on a Unix socket that misbehaves the way the tunnel does -- see `incus.md`. Issue #774 tracks the remaining tiers (a vsock-bridged QEMU boot, an aarch64 boot, and whether vfkit can run on GitHub's macOS runners at all). Anything touching the kernel config, the console, or the boot path needs a manual boot on both arches before it ships.
