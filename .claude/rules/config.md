...
**Project-local is untrusted**: it is whatever a cloned repo ships. Project-local templates' host-resources are confined to the project directory (`ImageDef.projectRoot` stamped at load, never from YAML; enforced in `HostResourceSetup.collectEffective()`, re-checked via `verifyConfined()` at build, branch, and start; recorded in `BuildSource.projectRoots`). Project-local tools cannot declare `proxy:` (`ToolProxyResolver.rejectProjectLocalProxy()`). Any new capability that reaches host state outside the project, such as host paths, host-only sources, or credentials, needs the same gate. See DESIGN.md "Project-local templates are confined to their project".
