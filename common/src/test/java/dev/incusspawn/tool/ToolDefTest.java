...
class ToolDefTest {
    ...
    @Test
    void parseProxyConfigEntryHelp() throws Exception {
        var def = ToolDef.loadFromStream(toStream("""
                name: help-tool
                proxy:
                  configuration:
                    api-key:
                      ...
                      help:
                        ...
                        - "  1. Go to https://example.com/keys"
                        - "  2. Click 'Create key'"
                  auth:
                    - domains:
                        - api.example.com
                      ...
                      token: "${api-key}"
                """));
        var help = def.getProxy().getConfiguration().get("api-key").getHelp();
        assertEquals(3, help.size());
        assertEquals("To create an API key:", help.get(0));
        assertEquals("  2. Click 'Create key'", help.get(2));
    }
