...
**Disk-space metrics**: An always-present **header band** (`renderHeader`) sits above the panels: a bold accent "brand chip" (` isx ` reverse-video, plus dim version) anchors app identity on the left, and a compact **storage gauge** is right-aligned on the same row when pool usage is available (fed by `IncusClient.getPoolUsageBytes(findUsablePool())`, cached in `poolUsage`, refreshed once per `reloadData()` — never per frame). Gauge fill colour is green/amber/red at the `STORAGE_WARN_PERCENT`/`STORAGE_CRIT_PERCENT` thresholds; at critical it also raises a one-shot status warning. The gauge bar grows with the terminal (`HEADER_BAR_MIN`..`HEADER_BAR_MAX`, ~width/5). Between the two, a quiet **"N running" badge** (`runningSummary`, split by kind — e.g. `● 2 containers, 1 VM running`, counted across `allEntries` by `runningCounts`) fills the gap; it is lower priority than the gauge and is shown only when it fits the leftover space, so it never evicts the gauge. On narrow terminals the badge drops first, then the gauge (then its bar), so the brand always survives; the gauge is simply omitted on `dir` pools that report no usage. Both panels carry a **DISK** column showing per-instance/per-template used bytes, parsed from `state.disk.<dev>.usage` in the existing `recursion=2` listing (`sumDiskUsage`); values render via `diskCell` as `~3.1G` (or `-` when unknown — `dir` pools and some stopped instances report nothing). Because btrfs reports each row's `usage` as *exclusive* bytes (blocks unique to that subvolume), the imported base image and all CoW-shared blocks belong to no row and surface only in the pool total. `foldBaseWeightIntoRootTemplate()` attributes that remainder — `sharedBaseBytes(poolUsage.usedBytes, Σ row usage)` — to the single root template (a built template with no parent), so the base reads at its real weight, clones show only their unique delta, and the rows roughly reconcile with the gauge. It runs only for CoW pools (`usagePoolIsCow`) and skips folding when the root is ambiguous (zero or several parentless templates). The `~` prefix and the delete-confirmation note (`cowDeleteNote` — which for the folded base template, tracked in `baseTemplateName`, warns the space is shared with everything derived from it) signal the approximation. The `C` key reclaims space by suspending the TUI and running `isx clean pool` via the pending-action pattern; on macOS the critical-fill warning also points at `isx vm resize` to grow the appliance disk.
