...
**Parallel repo cloning**: `BuildCommand.cloneRepos()` clones a template's declared repos concurrently, bounded to `CpuInfo.highPerfCores()` (`util/CpuInfo.java` — P-core count via macOS `sysctl`/Linux `cpu_capacity`, else all logical CPUs). `CpuInfo` is the single source of CPU-topology counts: `logicalCores()` (real host count, bypassing the native image's `-R:ActiveProcessorCount` cap; `ResourceLimits.hostProcessorCount()` delegates to it), `performanceCores()` (P/big cores, or 0 when indistinguishable; `VmManager.detectCpus()` uses it), and `highPerfCores()`. Clones and `prime` commands use captured (non-streamed) exec so parallel output doesn't garble; progress renders via `util/TerminalProgress.java`, the shared animated braille-spinner display also used by `HostRepoRefresh`'s parallel fetch. Incus device add/remove (host-reference mounts) must not run concurrently, so they bracket the parallel section in serial phases: mount-all → clone-all-in-parallel → remove-all → prime-in-parallel. Clone/prime failures are aggregated and abort the build.
