...
**Disk-space metrics**: A frameless full-width **storage gauge** renders above the panels, fed by `IncusClient.getPoolUsageBytes(findUsablePool())` (cached in `poolUsage`, refreshed once per `reloadData()` — never per frame). Fill colour is green/amber/red at the `STORAGE_WARN_PERCENT`/`STORAGE_CRIT_PERCENT` thresholds; at critical it also raises a one-shot status warning and the strip hides itself when no pool usage is available. Both panels carry a **DISK** column showing per-instance/per-template used bytes, parsed from `state.disk.<dev>.usage` in the existing `recursion=2` listing (`sumDiskUsage`); values render via `diskCell` as `~3.1G` (or `-` when unknown — `dir` pools and some stopped instances report nothing). Because storage is a single thin-provisioned CoW pool, per-row figures are approximate and **do not sum to the gauge** (shared blocks are double-counted, the image cache belongs to no row); the `~` prefix, a dim legend on the strip, the instance detail modal ("shares blocks with <parent>"), and the delete-confirmation note (`cowDeleteNote`) all signal this. The `C` key reclaims space by suspending the TUI and running `isx clean pool` via the pending-action pattern; on macOS the critical-fill warning also points at `isx vm resize` to grow the appliance disk.
