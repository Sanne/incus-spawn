...
**Pi coding agent support:** Pi is a provider-agnostic coding agent with a configurable `provider` parameter (default: `anthropic`). When using the default Anthropic provider, Pi communicates via the standard Anthropic API (`/v1/messages`) — the proxy handles direct key injection, OAuth Bearer injection, or standard-to-Vertex translation transparently, using `ANTHROPIC_API_KEY=sk-ant-placeholder` (or `ANTHROPIC_OAUTH_TOKEN` in OAuth mode). When `provider: openai` is set, Pi communicates with `api.openai.com` instead, using `OPENAI_API_KEY=sk-placeholder` — the proxy injects the real OpenAI API key via Bearer token replacement. Vertex/Google providers set `GOOGLE_CLOUD_PROJECT` and `GOOGLE_CLOUD_LOCATION` from the host Vertex config. All credential injection is declared via `PiSetup.envEntries()`, which selects the appropriate env vars based on the provider parameter. `PiSetup` declares OpenAI credentials as an optional proxy (`optionalProxy()`) so that `isx init` offers to configure them even when the `openai` feature flag is not enabled and Codex is not installed.
...
**Credential validation**: Building a template image that includes `claude`, `codex`, `pi`, or `gh` tools requires the corresponding credentials to be configured on the host. Both the CLI and TUI check this before starting a build and abort with a clear error if credentials are missing. Pi's required credential depends on its `provider` parameter: `anthropic` (default) requires an Anthropic key/OAuth/Vertex, `openai` requires an OpenAI API key, and `vertex`/`google` requires Vertex AI configuration. Tools behind a feature flag that is not enabled are excluded from credential checks.
...
**Dynamic credential setup:** `isx init` builds its credential menu dynamically from tool setups via `ToolDefLoader.allToolSetups()`. Tools with proxy configuration are discovered, filtered by feature gates (`ToolSetup.feature()` + `SpawnConfig.isFeatureEnabled()`), and sorted (known tools first: claude, gh, bob, codex; then alphabetically). Tools can also declare optional proxy credentials via `ToolSetup.optionalProxy()` — these bypass feature gates and appear in the menu with an `[optional]` tag when no other tool in the menu already covers the same config namespace. This ensures that tools like Pi (which can optionally use OpenAI credentials) allow credential setup even when the `openai` feature is not enabled. Each entry shows `ToolSetup.description()` and a `[configured]` tag. Known tools dispatch to specialized setup methods with validation (e.g., `setupClaudeAuth` with env-var detection and API verification); unknown tools use a generic prompt (`setupGenericToolCredentials`) that iterates proxy configuration directly (falling back to `optionalProxy()` when `proxy()` is null), respects `ConfigEntry.isSecret()` and `ConfigEntry.isConfirm()` (for y/n prompts like license acceptance), and saves via `SpawnConfig.setConfigByPath()` using the entry's `config-path`. User-defined tool YAMLs with proxy entries appear in the menu automatically.
...
| Pi credentials (provider-dependent) | Anthropic: placeholder `sk-ant-placeholder` in `ANTHROPIC_API_KEY` (or `ANTHROPIC_OAUTH_TOKEN`). OpenAI: placeholder `sk-placeholder` in `OPENAI_API_KEY`. Vertex/Google: `GOOGLE_CLOUD_PROJECT` + `GOOGLE_CLOUD_LOCATION` | Provider selected via `provider` parameter (default: `anthropic`). Same proxy injection as Claude/OpenAI respectively |
