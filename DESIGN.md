...
A missing stamp also self-heals on reload without a rebuild: `fillMissingReferencedSizes` backfills it with a single live probe — using the light (non-sync) flavour, and *only* when there's an actual gap (an unstamped built template). This keeps the privileged read out of the healthy per-refresh path (a fully-stamped install never probes on reload) while recovering pre-feature templates and the rare failed stamp; the backfill is in-memory (a rebuild re-stamps permanently) and never overwrites an existing stamp, which is authoritative and free. The `C` key reclaims space via `isx clean pool` (failed builds, unused images, build caches). Labelled "Storage" rather than "pool" to avoid leaking the Incus term, and the gauge/column degrade gracefully to hidden/`-` on `dir` pools that report no per-volume usage.
