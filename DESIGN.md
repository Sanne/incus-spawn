...
- **Known gap: DNS rebinding.** The name is resolved once for the check and again by `HttpClient` for the connection, so a hostile DNS server could answer differently each time. Exploiting it needs a domain whose DNS the attacker controls, with a short TTL. Closing it means connecting to the checked address while keeping TLS SNI and certificate verification on the name, which `HttpClient` does not support directly, so it is left as a follow-up.
