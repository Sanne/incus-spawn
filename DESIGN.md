...
**Host-only file sources**: A `repos[*].url` using `file:///absolute/host/path/.git` selects a required local source. `HostRepoSource` parses the URI without remote-URL case folding, validates a self-contained Git directory (bare repositories also work), and captures HEAD, symbolic branch, refs, and repository remote/branch configuration through Git, including local config includes. Linked worktree gitfiles, shallow/partial repositories, and alternates are rejected initially. `branch:` is rejected because host HEAD defines the checkout. Host refresh excludes these sources unconditionally.

The build uses the same serial read-only mount / parallel clone-and-prime / serial unmount pipeline as reference optimization, including host-to-appliance path translation. A required mount failure aborts the build and unwinds earlier mounts. VM sources wait for virtiofs attachment. Host-only preparation performs `git clone --local --mirror --no-hardlinks` into the destination's `.git`, changes `core.bare` to false, removes the generated remote config without deleting refs, restores symbolic refs and captured remote/branch settings, and populates the index/worktree at the captured HEAD. Mirror cloning preserves the host's ref namespaces; an ordinary clone would misleadingly map host branches to `origin/*`. Detached and unborn HEADs are preserved. The clone is self-contained after unmounting; no worktree changes, host index, hooks, or unrelated config are imported. Git runs without global/system configuration, with lazy object fetching disabled and only local file transport permitted. There is no remote fetch, remote HEAD discovery, or network fallback. A connectivity check and a second source metadata capture detect missing objects and source changes during copying, which fail preparation before prime.

`ImageDef.contentFingerprint()` includes host HEAD/branch, refs and copied configuration; build stamps use the captured fingerprint rather than rereading a potentially advanced host after priming. An unavailable source produces an outdated fingerprint for display and a specific error during build. File-source clones carry `isx.hostOnly=true` so automatic template/project Git maintenance skips them even if their YAML changes later. Explicit user Git operations and prime commands remain unrestricted. Branching only copies the template. Auto-remotes and TUI source lookup resolve the explicit Git directory directly; stateless remote cleanup includes file sources in available template definitions as well as configured search paths.
