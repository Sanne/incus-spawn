...
**Redirect rule cleanup:** `isx proxy uninstall` removes the systemd service **and** cleans up the PREROUTING redirect rule and bridge DNS overrides unconditionally — even if the service removal itself fails. `ProxyConfig.clearRedirectRules()` dispatches to the active firewall: on firewalld it queries `--direct --get-all-rules`, extracts the gateway IP from the matching PREROUTING rule, removes the permanent direct rule, and reloads; on UFW it reads `before.rules`, derives the subnet from the marked NAT block, and replaces the block with a redirect-free version (preserving the MASQUERADE rule that containers need for general internet access). If the subnet cannot be derived (malformed block), the entire NAT block is removed. `ProxyConfig.clearBridgeDns()` strips `address=` overrides from `incusbr0`'s `raw.dnsmasq`, preserving any `server=` entries. `isx reset` runs the same cleanup (and also destroys all containers, templates, VM, and host data).
