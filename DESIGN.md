...
A single shared proxy on `DEFAULT_MITM_PORT` serves every instance and holds one `ProxyCredentials`, so *per-instance* account selection is not yet possible — it needs the proxy to identify the calling instance (source address on the bridge, or a per-instance marker). The schema is shaped for it: accounts are named, and the name is the stable handle an instance would record.
