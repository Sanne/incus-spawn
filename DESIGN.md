...
**What can be swapped live, and what cannot.** Credentials never enter a container, so re-pointing an instance between accounts of the same kind takes effect on its next request with nothing inside restarting. Crossing Claude auth modes cannot work that way: `ClaudeSetup.envEntries()` writes a *different set of variables* for each mode into `/etc/profile.d/isx-env.sh` at build time, so a swap would leave the container describing an auth mode it is no longer using. `ToolSetup.envClass()` declares that — Claude returns its `effectiveType()`, everything else returns `""` and is therefore freely interchangeable, which is the right default since the generic path only substitutes a header. The class is stamped at build as `user.incus-spawn.env-class.<namespace>` and compared at selection time, so the swap is refused while the user is choosing rather than surfacing later as a failing request inside a container.
