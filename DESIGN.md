...
**AI help credentials (`isx ask` / `?` in the TUI):** `AiHelpClient.detectProvider()` picks a provider from `config.yaml` in the order API key > Vertex > OpenAI. A Claude Pro/Max OAuth token is deliberately **not** a candidate, even though `ClaudeConfig.hasAuth()` counts it as configured credentials. Such a token is only valid for Claude Code itself: the Messages API rejects requests carrying it unless `system` is a block array whose *first* block is Claude Code's own identity string, and it rejects them with an opaque HTTP 429 `rate_limit_error` whose message is the literal text `Error` — so the failure surfaces as a generic API error rather than an auth hint. Satisfying that check would mean isx presenting itself as Claude Code to spend the user's subscription, so `isx ask` instead falls through to the next configured provider, or explains the situation via `AiHelpClient.noProviderMessage()`. This is the one place isx calls a model API on its own behalf; everywhere else (`ProxyCredentials`, `ClaudeSetup`, `PiSetup`) the OAuth token is only forwarded into instances, where real Claude Code uses it. Because providers answer errors with varying usefulness, `describeError()` reports the HTTP status, the error `type`, and the `request_id` alongside the message.
